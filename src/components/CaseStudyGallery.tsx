...
import { Search, Gavel, Scale, ChevronRight, X, Filter, FolderPlus } from 'lucide-react';
      ...
      <div className="max-w-[1600px] w-full mx-auto px-4 sm:px-8 md:px-12 lg:px-16">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-slate-600 text-sm sm:text-base">
            Detailed breakdowns of litigation victories, court orders, and landmark appeals handled by Advocate Bhavni Singh.
          </p>
        </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-10 bg-slate-50 p-4 border border-slate-200">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
              {[
                ...
                { id: 'writ', label: 'Writ Petitions' },
                { id: 'criminal', label: 'Criminal & Bail' },
                { id: 'civil', label: 'Revenue & Land' },
        ...
        {caseStudies.length > 0 ? (
              ...
              >
                <div>
                  ...
                  <div className="p-3 bg-slate-50 border border-slate-200 text-xs text-slate-800 font-medium mb-4">
                    <strong className="text-[#1e293b]">Outcome:</strong> {cs.verdictOutcome || cs.outcome}
                  </div>
                </div>

                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-[11px] text-slate-400 italic">{cs.clientAnonymized || cs.court}</span>
                  <button
          ...
          <div className="text-center py-12 px-6 bg-slate-50 border border-dashed border-slate-300 space-y-3">
            <FolderPlus className="w-10 h-10 text-[#c5a059] mx-auto" />
            <p className="text-slate-800 text-base font-serif font-bold">No Case Studies Added Yet</p>
            <p className="text-slate-600 text-xs sm:text-sm max-w-md mx-auto">
              Case studies and High Court precedent analysis added by Advocate Bhavni Singh through the Chamber Portal will automatically appear in this gallery.
            </p>
          </div>
        )}

        {caseStudies.length > 0 && filteredCases.length === 0 && (
          <div className="text-center py-12 bg-slate-50 border border-dashed border-slate-300">
            ...
            <button
              ...
              className="mt-3 text-xs text-[#c5a059] font-bold underline cursor-pointer"
            >
              Reset filters
            </button>
          </div>
          ...
          <div className="relative w-full max-w-3xl bg-white shadow-xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto border border-slate-200">
            ...
            <div className="space-y-4 text-sm text-slate-600 leading-relaxed">
              <div className="bg-slate-50 p-4 border border-slate-200">
                ...
                <p className="text-slate-800">{selectedCase.challenge || selectedCase.summary}</p>
              </div>
              ...
              <div className="bg-slate-50 p-4 border border-slate-200">
                ...
                <p className="text-slate-900 font-semibold">{selectedCase.verdictOutcome || selectedCase.outcome}</p>
              </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
              <span className="text-xs text-slate-500 italic">{selectedCase.clientAnonymized || selectedCase.court}</span>
              <button
                onClick={() => {
